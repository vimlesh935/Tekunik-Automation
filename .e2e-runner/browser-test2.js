/* Browser E2E v2 — robust assertions using document.body.innerText. */
const { launchChrome, attachToPage, evalJs, waitFor, shutdownChrome, sleep } = require("./cdp");
const API = "http://localhost:8787";
const fs = require("node:fs");
const OUT = "browser-test2.out.txt";
let pass = 0, fail = 0;
...
(async () => {
  ...
  let token;
  try {
    const email = `btv2.${ts}@teknode.test`;
    await apiNode("POST", "/api/auth/register", { body: { email, password: "Passw0rd!123", username: `btv2_${ts}`, first_name: "B", last_name: "T", phone: "9999999997", age: 30, address: "1 F", city: "Indore", pincode: "452001" } });
    ...
    const targets = await launchChrome({ port: 9243 });
    ...
    if (save10Btn) {
      ...
      check("TEST 3b - 'Coupon applied' on page", pageText && /Coupon applied/i.test(pageText), pageText?.slice(0,150) || "no text");
      check("TEST 4 - 'You save' with 500 on page", pageText && /You save.*500|500.*save/i.test(pageText), pageText?.match(/You save[^\n]{0,30}/)?.[0] || "no match");
      check("TEST 5 - Total 2500.00 on page (6000 gross - 3000 offer - 500 coupon)", pageText && /2,500\.00/.test(pageText), pageText?.match(/Total Amount[^\n]{0,40}/)?.[0] || "no match");
    }
    ...
    await evalJs(client, `(() => { const i=document.querySelector('input[placeholder="Enter coupon code"]'); if(i){ i.click(); i.value='DOESNOTEXIST123'; i.dispatchEvent(new Event('input',{bubbles:true})); } return !!i; })()`);
    await sleep(500);
    ...
    await sleep(2500);
    const pageTextAfterFake = await evalJs(client, `(() => document.body.innerText)()`);
    const fakeOk = pageTextAfterFake && /not found/i.test(pageTextAfterFake);
    check("TEST 8 - Fake code 'not found'", fakeOk, pageTextAfterFake?.match(/not found[^\n]{0,60}/)?.[0] || "no error");

    // TEST 9: FUTURE50 -> "not active yet"
    await evalJs(client, `(() => { const i=document.querySelector('input[placeholder="Enter coupon code"]'); if(i){ i.value='FUTURE50'; i.dispatchEvent(new Event('input',{bubbles:true})); } return true; })()`);
    await sleep(500);
    await evalJs(client, `(() => { const btn=Array.from(document.querySelectorAll('button')).find(x=>/Apply/i.test(x.textContent) && !x.querySelector('svg')); if(btn){btn.click();} return true; })()`);
    await sleep(2500);
    const pageTextAfterFuture = await evalJs(client, `(() => document.body.innerText)()`);
    const isNotStarted = /not active yet/i.test(pageTextAfterFuture);
    check("TEST 9 - FUTURE50 'not active yet' (NOT not found)", isNotStarted, pageTextAfterFuture?.match(/not (active yet|found)[^\n]{0,60}/)?.[0] || "no error");
    ...
    try {
      ...
      const apiEmail = `api.${apiTs}@teknode.test`;
      await apiNode("POST", "/api/auth/register", { body: { email: apiEmail, password: "Passw0rd!123", username: `api_${apiTs}`, first_name: "A", last_name: "T", phone: "9999999998", age: 30, address: "1 F", city: "Indore", pincode: "452001" } });
      ...
      // Min order: FLAT200 needs 1499, cart = 1000
      await apiNode("POST", "/api/coupons/remove", { token: apiToken });
      ...
      log(`\n========================================`);
      log(`Browser E2E v2 RESULT  PASS=${pass} FAIL=${fail}`);
      log(`========================================`);
      process.exit(0);
    }
  }
