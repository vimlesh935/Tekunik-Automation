...
import { useNotificationFilterTypes } from "../utils/notificationTypes.js";

export default function Notifications() {
  const [filter, setFilter] = useState("");
  const { addToast } = useToast();
  const { t } = useTranslation();
  const filterTypes = useNotificationFilterTypes();
  ...
  return <div className="min-h-screen bg-page py-14 text-primary"><div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
    ...
    <div className="mb-6 flex gap-2 overflow-x-auto border-b border-slate-800 pb-3">{filterTypes.map((item) => <button key={item.value} type="button" onClick={() => setFilter(item.value)} className={`shrink-0 rounded-lg px-3 py-2 text-xs font-bold transition ${filter === item.value ? "bg-cyan-600 text-white" : "text-slate-400 hover:bg-slate-800 hover:text-white"}`}>{item.label}</button>)}</div>
