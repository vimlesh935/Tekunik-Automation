  ...
  return (
    <svg viewBox="0 0 440 600" style={{ width: "100%", height: "100%", position: "absolute", inset: 0 }} aria-hidden="true">
      <defs>
        ...
        <filter id="blur4c"><feGaussianBlur stdDeviation="4" /></filter>
      ...
      {edges.map(([a, b], i) => (
        <line key={i} x1={nodes[a].cx} y1={nodes[a].cy} x2={nodes[b].cx} y2={nodes[b].cy}
          stroke="url(#edgeGrad3)" strokeWidth="1"
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <input
        ...
        autoComplete={name} autoFocus={autoFocus} placeholder={focused ? placeholder : ""}
      ...
      {right && (
        <div style={{
          ...
          transition: "color 0.25s", fontSize: 16,
        }}>{right}</div>
      )}
    </div>
  );
...
function inputStyle(focused) {
  return {
    ...
    paddingTop: 18, paddingBottom: 6, paddingLeft: 44,
    background: focused ? "rgba(124,58,237,0.06)" : "rgba(255,255,255,0.025)",
    border: `1px solid ${focused ? V : BORDER}`, borderRadius: 14,
    color: TEXT, fontFamily: "'Space Grotesk', sans-serif", fontSize: 14,
      ...
      {steps.map((s, i) => (
        <div key={s} style={{ display: "flex", alignItems: "center", gap: 8, flex: i < 2 ? 1 : "none" }}>
          <div style={{
            ...
            fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 600, flexShrink: 0,
                      ...
                      <div className="otp-row">
                        {otp.map((digit, index) => (
                          <input key={index}
                            ...
                            placeholder="·" className="otp-box" autoFocus={index === 0} />
                        ))}
                      </div>
                    </Appear>
                    {error && (
  ...
  return (
    <button type="submit" disabled={loading}
      ...
      style={{
        ...
        background: loading ? BORDER : hover ? `linear-gradient(135deg, #6D28D9, #0891B2)` : `linear-gradient(135deg, ${V}, ${C})`,
        ...
        boxShadow: hover && !loading ? `0 8px 32px rgba(124,58,237,0.45), 0 0 0 1px rgba(124,58,237,0.2)` : `0 4px 16px rgba(124,58,237,0.25)`,
...
function FooterLink({ text, cta, onClick }) {
  ...
  return (
    <div style={{
      marginTop: 24, padding: "18px", background: "rgba(255,255,255,0.02)",
