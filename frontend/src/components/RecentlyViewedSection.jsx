...
export default function RecentlyViewedSection({
  ...
  const confirmClear = () => {
    if (window.confirm(t('common.clearAll') + "?")) onClear?.();
  };
    ...
    <section className={compact ? "rounded-xl bg-slate-900 border border-slate-800/80 p-5 sm:p-7 shadow-xl" : "border-y border-slate-900 bg-slate-900/40 py-20"}>
      <div className={compact ? "mb-6 flex items-start justify-between gap-4 border-b border-slate-800 pb-5" : "mx-auto mb-10 flex max-w-7xl items-end justify-between gap-4 px-6"}>
        <div>
          <div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-cyan-400">
            <Clock3 size={14} /> {t('dashboard.recentlyViewed')}
          </div>
          {!compact && <h2 className="text-3xl font-bold tracking-tight text-white">{t('recentlyViewed.heading')}</h2>}
          {compact && <p className="text-xs text-slate-400">{t('recentlyViewed.compactTitle')}</p>}
        </div>
      ...
      {products.length === 0 ? (
        <div className={compact ? "rounded-xl border border-dashed border-slate-800 p-8 text-center" : "mx-auto max-w-xl px-6 text-center"}>
          ...
          <p className="mt-2 text-xs leading-relaxed text-slate-500">{t('recentlyViewed.emptyDescription')}</p>
            ...
            <article key={product.id} className={compact ? "group relative rounded-lg border border-slate-800 bg-slate-950 p-3" : "group relative min-w-[250px] snap-start rounded-xl border border-slate-800 bg-slate-950 p-3 sm:min-w-[280px]"}>
              <Link to={`/product/${product.id}`} className="block">
                <div className="relative flex h-40 items-center justify-center overflow-hidden rounded-lg border border-slate-800/70 bg-slate-900">
                  ...
                  {hasDiscount(product) && <span className="absolute left-2 top-2 rounded bg-emerald-500/15 px-2 py-1 text-[10px] font-bold text-emerald-300">{t('coupon.percentOff', { percent: Math.round(product.discount_percent) })}</span>}
                </div>
                ...
                <div className="mt-2 flex items-end justify-between gap-2">
                  ...
                  <span className={`text-[10px] font-bold uppercase ${Number(product.stock_quantity) > 0 ? "text-emerald-400" : "text-rose-400"}`}>{Number(product.stock_quantity) > 0 ? t('product.inStock') : t('cart.unavailable')}</span>
                </div>
              </Link>
              <div className="mt-3 flex items-center gap-2 border-t border-slate-800/70 pt-3">
                {onAddToCart && <button type="button" onClick={(event) => onAddToCart(product, event)} disabled={Number(product.stock_quantity) <= 0} aria-label={`${t('product.addToCart')} ${product.name}`} className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-cyan-500/20 bg-cyan-500/10 py-2 text-[11px] font-bold text-cyan-300 transition hover:bg-cyan-500 hover:text-white disabled:cursor-not-allowed disabled:opacity-40"><ShoppingCart size={13} /> {t('product.addToCart')}</button>}
                {onToggleWishlist && <button type="button" onClick={() => onToggleWishlist(product.id)} aria-label={`${wishlist.includes(Number(product.id)) ? t('product.removeFromWishlist') : t('product.addToWishlist')} ${product.name}`} className="rounded-lg border border-slate-700 px-2.5 py-2 text-xs text-slate-400 transition hover:border-rose-400/40 hover:text-rose-300">{wishlist.includes(Number(product.id)) ? "♥" : "♡"}</button>}
                {onRemove && <button type="button" onClick={() => onRemove(product.id)} aria-label={t('recentlyViewed.removeAria', { name: product.name })} className="rounded-lg border border-slate-700 px-2.5 py-2 text-slate-500 transition hover:border-rose-400/40 hover:text-rose-300"><X size={14} /></button>}
              </div>
            </article>
          ))}
        </div>
